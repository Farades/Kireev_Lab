...
double D_y()
{
	...
	double V_average = V_01; 

	for (int i = 0; i < 3; i++)
	{
		//Расчет угла упреждения:
		double psi = asin( sin( (V_target * sin(q)) / V_average ) );
...
void main()
{
	...
	double mu_z_ = mu_z / xi_z_2;
	double mu_y_ = mu_y / xi_y_2;
	std::cout << "mu_z_" << std::setw(6) << "= " << mu_z_ << std::endl;
	std::cout << "mu_y_" << std::setw(6) << "= " << mu_y_ << std::endl;

	//Обобщенный коэффициент корреляции:
	double mu = sqrt(1 - sqrt((1 - mu_z_ * mu_z_) * (1 - mu_y_ * mu_y_)));
	std::cout << "mu" << std::setw(9) << "= " << mu << std::endl;
